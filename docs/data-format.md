...
- `provenance.verifiedAt` 是在线验证时间，未执行在线验证时为 `null`。`method: seed` 标识上游导入记录；`deterministic` 表示作品身份或逐集规则由脚本按放送日期比对得出；`codex` 表示模型研究结果，evidence 保留其引用的来源。自动化追加逐集规则时会在 evidence 末尾附上说明，不改动原有证据。
