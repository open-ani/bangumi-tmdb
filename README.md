...
<!-- stats:start -->
基于 Bangumi Archive 快照 `2026-09-15`，Archive 共有 **30,924** 个动画条目，处理情况如下。此区块由定时任务自动更新（`pnpm cli stats`）。

| 状态 | 条目数 | 说明 |
| --- | --- | --- |
| 已建立映射 | **18,599** | 占 60.1%。电影 6,169，TV 指定到季 12,240，TV 仅确定作品 190 |
| 已分析但未能确定 | 2,773 | 730 条经人工与模型复核仍无法确定，理由见 [待定清单](docs/unresolved-mappings-2026-09-10.md)；2,043 条为定时任务判定证据不足，按退避策略重试 |
| 尚未分析 | 9,589 | 2 条放送日在近 180 天内或尚未放送，每轮定时任务都会处理；其余为 2020 年前（3,697）、无放送日期（4,590）等早期或冷门条目，定时任务按每轮固定的积压配额从新到旧逐步处理 |

已映射条目的来源：

| 来源 | 条目数 | 含义 |
| --- | --- | --- |
| `codex` | 11,281 | 模型联网检索并读取 TMDB 详情与季表后的研究结论，evidence 保留引用 |
| `deterministic` | 6,476 | 脚本按放送日期比对确认作品身份或生成逐集规则 |
| `seed` | 842 | 直接沿用 BangumiExtLinker 的对应关系，尚未独立核验 |
| `community` | 0 | 社区 PR 提交的人工映射 |

逐集对应：**16,455** 条映射带有逐集规则，共 **184,705** 条 Bangumi 章节 → TMDB 剧集/电影的对应，覆盖已映射条目本篇章节（251,010 话）的 73.6%。其余 2,144 条目前只有作品或季级映射，逐集关系由定时任务逐步补齐：能按放送日期确定性推导的直接写入，其余交给模型研究。

剧集图片：已映射本篇章节中 130,077 话（70.6%）在 TMDB 有剧集图；13,441 个条目每一话都有图，2,490 个条目一张也没有。逐条目、逐章节的明细见 Release 里的 `coverage.json`。
