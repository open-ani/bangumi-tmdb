...
| 状态 | 条目数 | 说明 |
| --- | --- | --- |
| 已建立映射 | **18,931** | 占 61.2%。电影 6,310，TV 指定到季 12,515，TV 仅确定作品 106 |
| 已分析但未能确定 | 2,808 | 730 条经人工与模型复核仍无法确定，理由见 [待定清单](docs/unresolved-mappings-2026-09-10.md)；2,078 条为定时任务判定证据不足，按退避策略重试 |
| 尚未分析 | 9,244 | 1 条放送日在近 180 天内或尚未放送，每轮定时任务都会处理；其余为 2020 年前（3,331）、无放送日期（4,609）等早期或冷门条目，定时任务按每轮固定的积压配额从新到旧逐步处理 |
...
| `deterministic` | 6,603 | 脚本按放送日期比对确认作品身份或生成逐集规则 |
| `seed` | 708 | 直接沿用 BangumiExtLinker 的对应关系，尚未独立核验 |
| `community` | 0 | 社区 PR 提交的人工映射 |

逐集对应：**16,927** 条映射带有逐集规则，共 **190,107** 条 Bangumi 章节 → TMDB 剧集/电影的对应，覆盖已映射条目本篇章节（254,088 话）的 74.8%。其余 2,004 条目前只有作品或季级映射，逐集关系由定时任务逐步补齐：能按放送日期确定性推导的直接写入，其余交给模型研究。

剧集图片：已映射本篇章节中 134,008 话（70.7%）在 TMDB 有剧集图；13,791 个条目每一话都有图，2,601 个条目一张也没有。逐条目、逐章节的明细见 Release 里的 `coverage.json`。

已知问题：9 条映射的 TMDB 目标已失效（404 或声明范围缺集），已标记待重研究，修正前仍按原样发布。
<!-- stats:end -->
