...
<!-- stats:start -->
基于 Bangumi Archive 快照 `2026-09-15`，Archive 共有 **30,917** 个动画条目，处理情况如下。此区块由定时任务自动更新（`pnpm cli stats`）。

| 状态 | 条目数 | 说明 |
| --- | --- | --- |
| 已建立映射 | **12,427** | 占 40.2%。电影 4,379，TV 指定到季 7,858，TV 仅确定作品 190 |
| 已分析但未能确定 | 870 | 730 条经人工与模型复核仍无法确定，理由见 [待定清单](docs/unresolved-mappings-2026-09-10.md)；140 条为定时任务判定证据不足，按退避策略重试 |
| 尚未分析 | 17,657 | 155 条放送日在近 180 天内或尚未放送，已在定时任务队列中；其余为 2020 年前（8,846）、无放送日期（4,602）等早期或冷门条目，留待离线批处理 |

已映射条目的来源：

| 来源 | 条目数 | 含义 |
| --- | --- | --- |
| `codex` | 7,590 | 模型联网检索并读取 TMDB 详情与季表后的研究结论，evidence 保留引用 |
| `deterministic` | 3,561 | 脚本按放送日期比对确认作品身份或生成逐集规则 |
| `seed` | 1,276 | 直接沿用 BangumiExtLinker 的对应关系，尚未独立核验 |
| `community` | 0 | 社区 PR 提交的人工映射 |

逐集对应：**7,873** 条映射带有逐集规则，共 **69,686** 条 Bangumi 章节 → TMDB 剧集/电影的对应，覆盖已映射条目本篇章节（189,726 话）的 36.7%。其余 4,554 条目前只有作品或季级映射，逐集关系由定时任务逐步补齐：能按放送日期确定性推导的直接写入，其余交给模型研究。

已知问题：58 条映射的 TMDB 目标已失效（404 或声明范围缺集），已标记待重研究，修正前仍按原样发布。
<!-- stats:end -->
...
2. 推导逐集规则：只有作品或季级映射的条目，若声明的 TMDB 范围集数与 Bangumi 本篇章节数一致、且可比对的放送日期全部在 ±1 天内，直接生成 `rules`。两边日期都按集序递增时，也接受至少 80% 的日期在 ±1 天内、其余不超过 7 天，或逐集间隔一致、整体平移小于一集间隔的情况。两边缺少可比日期时，只有模型研究已限定为单一集范围（或单集）的条目才按声明范围生成，并在 evidence 中注明未做日期核验。已有规则的条目在新话放送后仍须日期在 ±1 天内才追加。
...
4. 重新生成 README 的统计区块，验证后以 `openanibot` 提交到 `main`，随即发布 Release。
