...
<!-- stats:start -->
基于 Bangumi Archive 快照 `2026-09-15`，Archive 共有 **30,912** 个动画条目，处理情况如下。此区块由定时任务自动更新（`pnpm cli stats`）。

| 状态 | 条目数 | 说明 |
| --- | --- | --- |
| 已建立映射 | **12,312** | 占 39.8%。电影 4,349，TV 指定到季 7,773，TV 仅确定作品 190 |
| 已分析但未能确定 | 785 | 730 条经人工与模型复核仍无法确定，理由见 [待定清单](docs/unresolved-mappings-2026-09-10.md)；55 条为定时任务判定证据不足，按退避策略重试 |
| 尚未分析 | 17,852 | 349 条放送日在近 180 天内或尚未放送，已在定时任务队列中；其余为 2020 年前（8,846）、无放送日期（4,603）等早期或冷门条目，留待离线批处理 |

已映射条目的来源：

| 来源 | 条目数 | 含义 |
| --- | --- | --- |
| `codex` | 7,475 | 模型联网检索并读取 TMDB 详情与季表后的研究结论，evidence 保留引用 |
| `deterministic` | 3,561 | 脚本按精确日期比对确认作品身份或生成逐集规则 |
| `seed` | 1,276 | 直接沿用 BangumiExtLinker 的对应关系，尚未独立核验 |
| `community` | 0 | 社区 PR 提交的人工映射 |

逐集对应：**7,776** 条映射带有逐集规则，共 **68,989** 条 Bangumi 章节 → TMDB 剧集/电影的对应，覆盖已映射条目本篇章节（188,699 话）的 36.6%。其余 4,536 条目前只有作品或季级映射，逐集关系由定时任务逐步补齐：能按放送日期确定性推导的直接写入，其余交给模型研究。
