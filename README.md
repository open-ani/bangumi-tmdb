...
| 已分析但未能确定 | 2,199 | 730 条经人工与模型复核仍无法确定，理由见 [待定清单](docs/unresolved-mappings-2026-09-10.md)；1,469 条为定时任务判定证据不足，按退避策略重试 |
| 尚未分析 | 11,802 | 12 条放送日在近 180 天内或尚未放送，已在定时任务队列中；其余为 2020 年前（5,895）、无放送日期（4,595）等早期或冷门条目，留待离线批处理 |
...
剧集图片：已映射本篇章节中 123,300 话（71.1%）在 TMDB 有剧集图；12,470 个条目每一话都有图，2,075 个条目一张也没有。逐条目、逐章节的明细见 Release 里的 `coverage.json`。
