...
import { Catalog, Mapping, Progress, Seeds } from './model.js';
...
try {
  switch (command) {
    ...
    case 'publish': await publish(root); break;
    default: throw new Error('Usage: pnpm cli <import-seed [SHA]|archive|validate|format|update|verify|guard SHA|publish>');
  }
} catch (error) {
