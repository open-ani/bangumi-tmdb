...
// Every comparable pair must agree; most pairs must be comparable so a season without dates cannot pass.
export function datesAgree(pairs: { bangumi: Episode; tmdb: TmdbEpisode }[]): { compared: number; ok: boolean } {
  let compared = 0;
  for (const pair of pairs) {
    const delta = dayDelta(pair.bangumi.airdate, pair.tmdb.air_date);
    if (delta === null) continue;
    compared++;
    if (delta > MAX_DATE_DRIFT_DAYS) return { compared, ok: false };
  }
  return { compared, ok: compared >= Math.max(1, Math.ceil(pairs.length * 0.6)) };
}
...
export async function derive(mapping: Mapping, episodes: Episode[], tmdb: SeasonSource): Promise<Derived | null> {
  ...
  const { compared, ok } = datesAgree(regular.map((bangumi, i) => ({ bangumi, tmdb: flat[i]! })));
  if (!ok) return null;
  const rules: Rule[] = [];
  let index = 0;
  for (const s of segs) {
  ...
  expand(row);
  return { mapping: row, note: `脚本按放送日期核对生成逐集规则：${regular.length} 个本篇章节与声明的 TMDB 范围逐一对应，${compared} 对日期全部在 ±${MAX_DATE_DRIFT_DAYS} 天内。` };
}
