...
test('update derives, researches, respects scope and locks, and is idempotent', async () => {
  ...
  try {
    ...
    await writeFile(join(bin, 'codex'), `#!${process.execPath}\n` +
      ...
      `const id=JSON.parse(p.slice(p.indexOf('{"bangumi"'))).bangumi.id;if(id===8)process.exit(3);\n` +
    ...
    await writeJson(join(root, '.cache/catalog.json'), { snapshot: { name: 'dump-2026-09-08.210336Z.zip', sha256: 'a'.repeat(64), url: 'https://example.org/dump.zip' },
      subjects: [subject(1, '2026-07-01'), subject(2, '2026-08-01'), subject(3, '2010-01-01'), subject(4, '2026-05-01'), subject(5, '2026-06-01'), subject(8, '2026-06-15')],
      episodes: [ep(11, 1, 1, day('2026-07-01')), ep(12, 1, 2, '2026-07-08'), ep(13, 1, 3, '2026-07-15'), ep(21, 2, 1, '2026-08-01'), ep(22, 2, 2, '2026-08-08'), ep(31, 3, 1, '2010-01-01'), ep(41, 4, 1, '2026-05-01'), ep(51, 5, 1, '2026-06-01'), ep(52, 5, 2, '2026-06-08'), ep(81, 8, 1, '2026-06-15')],
      relations: [] });
    ...
    assert.deepEqual(Object.fromEntries(Object.entries(progress.subjects).map(([k, v]) => [k, v.status])), { '1': 'matched', '2': 'matched', '4': 'locked', '5': 'matched', '8': 'error' });
    ...
    assert.equal(report.derived, 1); assert.equal(report.resolved, 1); assert.equal(report.researchMatched, 1); assert.equal(report.codexSubjects, 2, 'the resolved subject never reaches the model');
    assert.equal(report.changed, 3); assert.equal(report.queued.unmapped, 3);
    assert.match(progress.subjects['8']!.reason, /^Research failed: Error: Codex exited 3/);
    assert.equal(progress.subjects['8']!.retryAt, new Date(now + 86400000).toISOString(), 'a process failure is retried the next day without backoff');
