...
test('only recent, upcoming or previously handled subjects are automation scope; retries back off', () => {
  ...
  const finished = { ...subject, date: '2026-03-01' };
  assert.equal(retryDays('pending', 1, finished, now), 7);
  assert.equal(retryDays('pending', 2, finished, now), 14);
  assert.equal(retryDays('pending', 9, finished, now), 90);
  assert.equal(retryDays('pending', 4, { ...subject, date: '2026-09-01' }, now), 7, 'a work that premiered within 30 days is looked at weekly');
  assert.equal(retryDays('pending', 4, subject, now), 56, 'after its first month the backoff takes over');
